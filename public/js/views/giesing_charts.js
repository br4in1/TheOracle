function plot_dnog(Type = 1){
  $.ajax({
    url: Routing.generate('get_Giesing_DNOG', { type: Type }),
  }).then(returned => {
...
function plot_dnog2(){
  $.ajax({
    url: Routing.generate('get_Giesing_DNOG_ByMonth'),
  }).then(returned => {
...
function plot_dnog3(){
  $.ajax({
    url: Routing.generate('get_Giesing_DNOG_ByDayOfWeek'),
  }).then(returned => {
...
function plot_polar(){
  $.ajax({
    url: Routing.generate('get_Giesing_Polarity'),
  }).then(returned => {

    $('#polarity').remove(); // this is my <canvas> element
    $('#polarity_container1').append('<canvas id="polarity"><canvas>');
    returned = JSON.parse(returned);
    var data = {
      labels: returned.labels,
      datasets: [{
